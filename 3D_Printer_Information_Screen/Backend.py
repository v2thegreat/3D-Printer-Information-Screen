...
class Backend(PrinterUI, BackendStatics):
	...
	MAKING_CHANGES_ONLY_FLAG: bool
	INCORRECT_STUDENT_OR_PERSONNEL_FLAG: bool
	INCORRECT_STUDENT_ID_FLAG: bool
	lastIDUsed: Union[str, Any]
	...
	def _settingFunctionality(self):  # setup GUI Button Function --Dependency: PrinterUI Class
		...
		"""
        Stage 3:
            This is the stage where the user uses the octoprint workstation to upload and start their prints. 
            While the time is for 5 mins, this time can be changed easily by changing the HIDE_DISPLAY_TIME parameter, 
            and even be switched to running the next stage by checking the display screen instead and 
            using it when the screen goes blank
        """
		# No functions defined, so leaving it empty
		...
		self.pb_St4_MakeChanges.clicked.connect(lambda: self._setMakeChangesFlag())
        ...
        """
		self.pb_St5_Num0.clicked.connect(
			lambda: self.addDigitForSt5(self.lbl_St5_StudentOrPersonnelID, self.pb_St5_Num0))
		self.pb_St5_Num1.clicked.connect(
			lambda: self.addDigitForSt5(self.lbl_St5_StudentOrPersonnelID, self.pb_St5_Num1))
		self.pb_St5_Num2.clicked.connect(
			lambda: self.addDigitForSt5(self.lbl_St5_StudentOrPersonnelID, self.pb_St5_Num2))
		self.pb_St5_Num3.clicked.connect(
			lambda: self.addDigitForSt5(self.lbl_St5_StudentOrPersonnelID, self.pb_St5_Num3))
		self.pb_St5_Num4.clicked.connect(
			lambda: self.addDigitForSt5(self.lbl_St5_StudentOrPersonnelID, self.pb_St5_Num4))
		self.pb_St5_Num5.clicked.connect(
			lambda: self.addDigitForSt5(self.lbl_St5_StudentOrPersonnelID, self.pb_St5_Num5))
		self.pb_St5_Num6.clicked.connect(
			lambda: self.addDigitForSt5(self.lbl_St5_StudentOrPersonnelID, self.pb_St5_Num6))
		self.pb_St5_Num7.clicked.connect(
			lambda: self.addDigitForSt5(self.lbl_St5_StudentOrPersonnelID, self.pb_St5_Num7))
		self.pb_St5_Num8.clicked.connect(
			lambda: self.addDigitForSt5(self.lbl_St5_StudentOrPersonnelID, self.pb_St5_Num8))
		self.pb_St5_Num9.clicked.connect(
			lambda: self.addDigitForSt5(self.lbl_St5_StudentOrPersonnelID, self.pb_St5_Num9))
		self.pb_St5_BackSpace.clicked.connect(lambda: self.removeDigit(self.lbl_St5_StudentOrPersonnelID))
		self.pb_St5_Enter.clicked.connect(
			lambda: self.enteredIDForStage5(self.lbl_St5_StudentOrPersonnelID, self.Dialog))
	...
	# Stage: All
	def _displayStage(self, stageNumber: int) -> None:  # GUI Function --Dependency: PrinterUI Class
		"""
        Args:
	...
	# Stage: 2,5
	def _GoBack(self, currentStage: int) -> None:  # GUI Function --Dependency: PrinterUI Class
		"""
		:param currentStage: int: Used to tell it the current stage so it can calculate the previous one
		:return: None

		Stage: 2,5
		Function to go back to a previous stage
		"""
		self._displayStage(currentStage - 2)

	def removeDigit(self,
	                studentIDLabel):  # Higher Level GUI Function --Dependency: Backend._setIncorrectLabelToDefault, Backend._isRemoveDigit, Backend._removeDigit
		"""
        Args:
		...
		self.INCORRECT_STUDENT_ID_FLAG = Backend._setIncorrectLabelToDefault(studentIDLabel,
		                                                                     self.INCORRECT_STUDENT_ID_FLAG,
	...
	def addDigitForSt2(self, studentIDLabel,
	                   btn):  # GUI Button Function --Dependency: PrinterUI Class, Backend._setIncorrectLabelToDefault, Backend._isAddDigit, Backend._addText
		"""
        Args:
		...
		if Backend.isAddDigitValid(studentIDLabel):
			Backend._addText(studentIDLabel, btn)

	def digitEntered(self, studentIDLabel,
	                 Dialog):  # Too Complex, save for later/leave for backend as too many dependencies
		"""
        Args:
	...
	def _setMakeChangesFlag(self):
		"""
		Args: None
		:return: None

		Stage: 4
		Function to setup flags for when the print needs to be modified
		"""
		self.MAKING_CHANGES_ONLY_FLAG = True
		self._displayStage(4)
	...
	def addDigitForSt5(self, studentIDLabel, btn):
		...
		self.INCORRECT_STUDENT_OR_PERSONNEL_FLAG = Backend._setIncorrectLabelToDefault(studentIDLabel,
		                                                                               self.INCORRECT_STUDENT_OR_PERSONNEL_FLAG,
		                                                                               DEFAULT_TEXT_lbl_St5_StudentOrPersonnelID)

		if Backend.isAddDigitValid(studentIDLabel):
			Backend._addText(studentIDLabel, btn)

	def enteredIDForStage5(self, studentOrPersonnelIDLabel,
	                       Dialog):  # Too Complex, save for later/leave for backend as too many dependencies
		"""
        Args:
            studentOrPersonnelIDLabel      (:QLabel:    QtWidgets.QLabel): A QLabel object which has the student ID that we need to get the student ID or personnel pin from
            Dialog              (:QDialog:    QtWidgets.QDialog): Dialog that we'll send to the back so that we can use the octoprint screen

        Stage: 5
        Function to verify that password for personnel is correct and to note that a cancelation has been made
            It sends the dialog back and resets the stage
            Finally Goes back to first stage
        """
		self.INCORRECT_STUDENT_OR_PERSONNEL_FLAG = Backend._setIncorrectLabelToDefault(studentOrPersonnelIDLabel,
		                                                                               self.INCORRECT_STUDENT_OR_PERSONNEL_FLAG,
		                                                                               DEFAULT_TEXT_lbl_St5_StudentOrPersonnelID)
		if self.isPersonnelOrStudentIDCorrect(studentOrPersonnelIDLabel):
			if self.MAKING_CHANGES_ONLY_FLAG:
				self.MAKING_CHANGES_ONLY_FLAG = False
				Backend.saveModificationRecord(studentOrPersonnelIDLabel)
				Backend.sendDialogBack(Dialog)
				studentOrPersonnelIDLabel.setText(DEFAULT_TEXT_lbl_St5_StudentOrPersonnelID)
				self._displayStage(3)
				print('Displayed Stage 0')
			else:
				Backend.saveCancelRecord(studentOrPersonnelIDLabel)
				Backend.sendDialogBack(Dialog)
				studentOrPersonnelIDLabel.setText(DEFAULT_TEXT_lbl_St5_StudentOrPersonnelID)
				self._displayStage(0)
				print('Displayed Stage 0')
			...
			self.INCORRECT_STUDENT_OR_PERSONNEL_FLAG = Backend.setStudentIDLabelAsErrorLabel(
				studentOrPersonnelIDLabel, STUDENT_ID_OR_PERSONNEL_ID_INCORRECT_MESSAGE)

	def isSameID(self, studentOrPersonnelIDLabel):  # Saving/Backend Function --Dependency:  Backend._getStdID
		"""
        Args:
            studentOrPersonnelIDLabel      (:QLabel:    QtWidgets.QLabel): A QLabel object which has the student ID that we need to use to check if the student's ID is the same as the one used to setup the print
        ...
        """
		return self.lastIDUsed == Backend._getStdID(studentOrPersonnelIDLabel)

	def isPersonnelOrStudentIDCorrect(self,
	                                  studentIDLabel):  # Saving/Backend Function --Dependency:  self.isSameID, Backend.isPersonnelPinCorrect, Backend._getStdID
		"""
        Args:
